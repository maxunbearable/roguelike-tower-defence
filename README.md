...
- **[`docs/superpowers/plans/`](docs/superpowers/plans/) — 35 engineering logs**,
  one per change, each ending in an execution log that records what actually
  happened rather than what was planned. They are the most useful thing here if
  you want to know how the game was built. A few of the findings:
  - The build menu offered a new profile a cannon it could afford but had not
    unlocked, and the only refusal message was *"not enough gold"* — the one
    explanation that was false.
  - `Loadout::ownAll` defaulted to `true` as scaffolding and outlived its
    replacement by twenty-odd plans, so every gameplay screenshot ever taken was
    of a fully upgraded board.
  - The skill tree drew a walked prerequisite link at **9 luma** of contrast
    against its own parchment, so it showed a new player its structure and hid
    it the moment they bought anything.
  - Three rounds' premises were **refuted by measuring them** and the planned
    fix was abandoned; those write-ups say so.
...
- [`docs/ASSET-POLICY.md`](docs/ASSET-POLICY.md) — licence clearances, including
  what was rejected after reading the licence at source.
