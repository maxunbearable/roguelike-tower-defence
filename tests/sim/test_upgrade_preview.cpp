...
TEST_CASE("the upgrade preview matches what upgrading actually produces",
    ...
    for (const auto& [towerId, def] : reg.towers()) {
        ...
        while (w.upgradeCost(PLOT(0)) > 0) {
            ...
            REQUIRE(w.previewUpgrade(PLOT(0), predicted));
            REQUIRE(w.upgradeTower(PLOT(0)));
            const auto& actual = w.reg().get<sim::TowerStats>(w.towerAt(PLOT(0)));

            UNSCOPED_INFO(towerId << " at level " << w.reg().get<sim::TowerTag>(w.towerAt(PLOT(0))).level);
        ...
        REQUIRE_FALSE(w.previewUpgrade(PLOT(0), unused));
    }
}
...
TEST_CASE("an upgrade preview improves the stats it claims to", "[towers][preview]") {
    ...
    sim::TowerStats next;
    REQUIRE(w.previewUpgrade(PLOT(0), next));
